...
class MasterScenario:
    ...
    terminate_within_distance_km: Optional[float] = None
    terminate_primary_agent: Optional[str] = None
    terminate_secondary_agent: Optional[str] = None
    ...
    def __post_init__(self) -> None:
        ...
        if self.terminate_within_distance_km is not None and self.terminate_within_distance_km <= 0.0:
            raise ValueError("terminate_within_distance_km must be positive when provided.")
        if (self.terminate_primary_agent is None) != (self.terminate_secondary_agent is None):
            raise ValueError("Set both terminate_primary_agent and terminate_secondary_agent, or neither.")
...
class RocketAgentConfig:
    ...
    pre_sim_policy: Optional[Policy] = None
    allow_pre_sim_maneuvers: bool = False
    allow_failed_insertion_handoff: bool = False
...
class MasterSimulator:
    ...
    def run(
        ...
        stage_agent_names = [
            self._stage_agent_name(cfg, stage.name) for cfg in rockets for stage in cfg.rocket.effective_stages()
        ]
        all_names = base_names + rocket_sat_names + stage_agent_names
        ...
        for k in range(max_total_steps):
            ...
            for rocket_cfg in rockets:
                ...
                insertion_time_s = t_now + (result.t_s[-1] - result.launch_delay_s)
                insertion_sat_name = rocket_cfg.inserted_satellite_name or f"{rocket_cfg.name}_inserted"
                ...
                log.launches.append(
                    RocketLaunchEvent(
                        ...
                        inserted_satellite_name=insertion_sat_name,
                        inserted=result.inserted,
                        insertion_reason=result.insertion_reason,
                    )
                )
                pending_insertions.extend(self._build_pending_stage_insertions(rocket_cfg, result, t_now))
                can_handoff_failed = self._failed_insertion_handoff_allowed(result)
                if result.inserted or rocket_cfg.allow_failed_insertion_handoff or can_handoff_failed:
                    ...
                    log.termination_reason = f"rocket_insertion_failed::{rocket_cfg.name}::{result.insertion_reason}"
                    stop_requested = True
                break
    ...
    def _termination_reason(self, active_agents: dict[str, SatelliteAgentConfig]) -> Optional[str]:
        ...
        prox_km = self.scenario.terminate_within_distance_km
        if prox_km is not None:
            a = self.scenario.terminate_primary_agent
            b = self.scenario.terminate_secondary_agent
            if a is not None and b is not None and a in active_agents and b in active_agents:
                ra = active_agents[a].satellite.state.x_eci[0:3]
                rb = active_agents[b].satellite.state.x_eci[0:3]
                sep = float(np.linalg.norm(ra - rb))
                if sep <= prox_km:
                    return f"agents_within_distance::{a}::{b}::{sep:.6f}km"

        return None

    def _failed_insertion_handoff_allowed(self, result: LaunchResult) -> bool:
        radius_km = float(np.linalg.norm(result.satellite.state.x_eci[0:3]))
        if not np.isfinite(radius_km):
            return False
        return radius_km >= EARTH_EQUATORIAL_RADIUS_KM
    ...
    def _stage_agent_name(self, cfg: RocketAgentConfig, stage_name: str) -> str:
        return f"{cfg.name}_{stage_name}"

    def _build_pending_stage_insertions(
        self,
        cfg: RocketAgentConfig,
        result: LaunchResult,
        launch_time_s: float,
    ) -> list[_PendingInsertion]:
        pending: list[_PendingInsertion] = []
        for event in result.stage_separations:
            stage_agent_name = self._stage_agent_name(cfg, event.stage_name)
            sat_params = SatParams(
                name=stage_agent_name,
                mu=result.satellite.params.mu,
                r0_eci_km=event.x_eci[0:3],
                v0_eci_km_s=event.x_eci[3:6],
                max_accel_km_s2=0.0,
                min_accel_km_s2=0.0,
                propellant_dv_km_s=np.inf,
            )
            stage_sat = Satellite.from_params(sat_params, strategy=None)
            activation_time_s = launch_time_s + (event.separation_time_s - result.launch_delay_s)
            stage_sat.state.t = activation_time_s
            pending.append(
                _PendingInsertion(
