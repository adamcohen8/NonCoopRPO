...
class Rocket:
    ...
    simple_insertion_check_period_s: float = 5.0
    attitude_control_enabled: bool = False
    inertia_body_kg_m2: Optional[np.ndarray] = None
    max_torque_nm: Optional[np.ndarray] = None
    stages: Optional[list["RocketStage"]] = None

    def initial_mass_kg(self) -> float:
        stages = self.effective_stages()
        return float(sum(stage.dry_mass_kg + stage.fuel_mass_kg for stage in stages))

    def mdot_kg_s(self, stage_index: int = 0) -> float:
        stages = self.effective_stages()
        idx = int(np.clip(stage_index, 0, len(stages) - 1))
        stage = stages[idx]
        if stage.isp_s <= 0.0:
            return 0.0
        return stage.thrust_newton / (stage.isp_s * G0_M_S2)

    def max_ideal_delta_v_km_s(self) -> float:
        stages = self.effective_stages()
        m = self.initial_mass_kg()
        dv_km_s = 0.0
        for stage in stages:
            if stage.dry_mass_kg <= 0.0 or stage.fuel_mass_kg < 0.0:
                continue
            m_after_burn = m - stage.fuel_mass_kg
            if m_after_burn <= 0.0 or m_after_burn >= m:
                continue
            dv_km_s += (stage.isp_s * G0_M_S2 / 1000.0) * np.log(m / m_after_burn)
            m = max(m_after_burn - stage.dry_mass_kg, 1e-9)
        return float(max(dv_km_s, 0.0))

    def effective_stages(self) -> tuple["RocketStage", ...]:
    ...
    def num_stages(self) -> int:
        return len(self.effective_stages())
...
class LaunchResult:
    ...
    stage_separations: list[StageSeparationEvent] = field(default_factory=list)
...
def _geodetic_to_ecef(lat_deg: float, lon_deg: float, h_km: float) -> np.ndarray:
    # Keep launch geometry consistent with the point-mass gravity model and
    # Earth-surface checks elsewhere in the simulator (spherical Earth).
    lat = np.deg2rad(lat_deg)
    lon = np.deg2rad(lon_deg)
    r = EARTH_EQUATORIAL_RADIUS_KM + h_km
    cos_lat = np.cos(lat)
    sin_lon = np.sin(lon)
    cos_lon = np.cos(lon)
...
def _launch_delay_for_mode(
    ...
    target_h_hat_nom = _unit(np.cross(target0_eci[0:3], target0_eci[3:6]))
    target_speed_nom_km_s = float(np.linalg.norm(target0_eci[3:6]))
    if target_speed_nom_km_s <= 1e-9:
        target_speed_nom_km_s = float(np.sqrt(mu / max(target_orbit.a_km, 1e-9)))
    ...
    def metrics_at(t_s: float) -> tuple[float, float, bool]:
        ...
        plane_err_deg = float(np.rad2deg(np.arcsin(np.clip(abs(np.dot(_unit(r_site), target_h_hat_nom)), 0.0, 1.0))))
        ...
        target_v_vec = target_speed_nom_km_s * horiz
    ...
    window_s = 86164.0905
    samples = 721
    if rocket.guidance_mode.strip().lower() in {"simple", "open_loop"}:
        samples = 181
...
def simulate_launch_to_insertion(
    ...
    stages = rocket.effective_stages()
    if len(stages) == 0:
        raise ValueError("Rocket must define at least one stage.")
    for idx, stage in enumerate(stages):
        if stage.dry_mass_kg <= 0.0:
            raise ValueError(f"Stage {idx + 1} dry_mass_kg must be positive.")
        if stage.fuel_mass_kg < 0.0:
            raise ValueError(f"Stage {idx + 1} fuel_mass_kg must be non-negative.")
        if stage.thrust_newton < 0.0:
            raise ValueError(f"Stage {idx + 1} thrust_newton must be non-negative.")
        if stage.isp_s < 0.0:
            raise ValueError(f"Stage {idx + 1} isp_s must be non-negative.")
        if stage.cd < 0.0:
            raise ValueError(f"Stage {idx + 1} cd must be non-negative.")
        if stage.area_m2 <= 0.0:
            raise ValueError(f"Stage {idx + 1} area_m2 must be positive.")
    AttitudeConstraint(
    ...
    simple_mode = guidance_mode in {"simple", "open_loop"}
    simple_check_stride = max(1, int(np.ceil(rocket.simple_insertion_check_period_s / max(dt_s, 1e-9))))
    total_fuel_kg = float(sum(stage.fuel_mass_kg for stage in stages))
    stage_fuel_kg = np.asarray([stage.fuel_mass_kg for stage in stages], dtype=float)
    stage_dry_kg = np.asarray([stage.dry_mass_kg for stage in stages], dtype=float)
    stage_stack_mass_kg = np.asarray([stage.dry_mass_kg + stage.fuel_mass_kg for stage in stages], dtype=float)
    suffix_stack_mass_kg = np.cumsum(stage_stack_mass_kg[::-1])[::-1]

    def vehicle_from_total_fuel(remaining_total_fuel_kg: float) -> tuple[Optional[int], float, float, RocketStage]:
        fuel_clamped = float(np.clip(remaining_total_fuel_kg, 0.0, total_fuel_kg))
        burned = total_fuel_kg - fuel_clamped
        cum = 0.0
        for i, fuel_i in enumerate(stage_fuel_kg):
            if burned < (cum + fuel_i):
                burned_in_stage = burned - cum
                remaining_in_stage = fuel_i - burned_in_stage
                attached_mass = suffix_stack_mass_kg[i] - burned_in_stage
                return i, float(remaining_in_stage), float(attached_mass), stages[i]
            cum += fuel_i
        # No active fueled stage left; keep final dry stage ballistic.
        return None, 0.0, float(stage_dry_kg[-1]), stages[-1]
    ...
    stage_events: list[StageSeparationEvent] = []
    ...
    for k in range(steps):
        ...
        t_rel = k * dt_s
        stage_idx_now, _stage_fuel_now, _mass_now, stage_now = vehicle_from_total_fuel(fuel_kg)
        mdot_now = rocket.mdot_kg_s(stage_idx_now) if stage_idx_now is not None else 0.0
        if not simple_mode:
        ...
        def deriv(local_t_rel_s: float, local_state: np.ndarray) -> np.ndarray:
            ...
            local_stage_idx, _local_stage_fuel, local_mass, local_stage = vehicle_from_total_fuel(local_fuel_kg)
            local_mdot = rocket.mdot_kg_s(local_stage_idx) if local_stage_idx is not None else 0.0
            r = local_x[0:3]
            v = local_x[3:6]
            r_norm = np.linalg.norm(r)
            mass = max(local_mass, 1e-9)
            a_grav = -mu * r / (max(r_norm, 1e-9) ** 3)

            rho = atmos76_density_from_eci(r)
            v_atm = np.cross(np.array([0.0, 0.0, EARTH_ROT_RATE_RAD_S], dtype=float), r)
            v_rel = v - v_atm
            speed = np.linalg.norm(v_rel)
            area_over_mass_km2_per_kg = (local_stage.area_m2 * 1e-6) / mass
            a_drag = -0.5 * local_stage.cd * area_over_mass_km2_per_kg * rho * speed * v_rel

            def throttle_for_state(state_x: np.ndarray, state_fuel_kg: float) -> float:
                state_stage_idx, _state_stage_fuel, _state_mass, _state_stage = vehicle_from_total_fuel(state_fuel_kg)
                state_mdot = rocket.mdot_kg_s(state_stage_idx) if state_stage_idx is not None else 0.0
                if simple_mode:
                    if state_stage_idx is not None and state_fuel_kg > 0.0 and state_mdot > 0.0:
                        return 1.0
                    return 0.0
                throttle_cmd_local = 0.0
                if phase != "coast_to_apogee" and state_stage_idx is not None and state_fuel_kg > 0.0 and state_mdot > 0.0:
            ...
            thrust_n = local_stage.thrust_newton * throttle_cmd if local_stage_idx is not None else 0.0
            a_thrust = (thrust_n / mass) * thrust_dir / 1000.0
            fuel_dot = -local_mdot * throttle_cmd if throttle_cmd > 0.0 else 0.0
            a_total = a_grav + a_drag + a_thrust
            return np.hstack((v, a_total, fuel_dot))

        if simple_mode:
            throttle_cmd_now = 1.0 if (stage_idx_now is not None and fuel_kg > 0.0 and mdot_now > 0.0) else 0.0
        else:
            throttle_cmd_now = 0.0
            if phase != "coast_to_apogee" and stage_idx_now is not None and fuel_kg > 0.0 and mdot_now > 0.0:
        ...
        thrust_log[k] = (stage_now.thrust_newton * throttle_cmd_now) if stage_idx_now is not None else 0.0
        ...
        stage_idx_after, _stage_fuel_after, _mass_after, _stage_after = vehicle_from_total_fuel(fuel_kg)
        if stage_idx_now is not None and (stage_idx_after is None or stage_idx_after > stage_idx_now):
            sep_end = stage_idx_after if stage_idx_after is not None else len(stages)
            for sep_idx in range(stage_idx_now, sep_end):
                stage_events.append(
                    StageSeparationEvent(
                        stage_name=stages[sep_idx].name,
                        stage_index=sep_idx,
                        separation_time_s=t_abs + dt_s,
                        x_eci=x.copy(),
                    )
                )
        ...
        check_insertion_now = True
        if simple_mode:
            check_insertion_now = (k % simple_check_stride) == 0

        if check_insertion_now and _insertion_met(x, insertion_criteria):
    ...
    return LaunchResult(
        ...
        stage_separations=stage_events,
    )
