...
# Allow running this file directly: `python examples/MVP_Harness.py`
...
def main() -> None:
    ...
    def chaser_policy(_t: float, x_rel_ric: np.ndarray, _x_eci: np.ndarray) -> np.ndarray:
        ...
        # HCW/LQR model convention uses opposite sign for in-track states
        # relative to the current RIC convention used in frames/sim.
        # x_model[1] *= -1.0
        # x_model[4] *= -1.0
        return lqr(x_model)
    ...
    print("Initial |rel| (km):", np.linalg.norm(log.rel_ric[0, :3]))
    print("Final |rel| (km):", np.linalg.norm(log.rel_ric[-1, :3]))
    ...
    print("Detection fraction:", float(np.mean(log.detected_by_chaser)))
    ...
    fig2 = plt.figure()
    plt.plot(log.t_s / 60.0, log.rel_ric[:, 3]*1000.0, label="dR (km/s)")
    plt.plot(log.t_s / 60.0, log.rel_ric[:, 4]*1000.0, label="dI (km/s)")
    plt.plot(log.t_s / 60.0, log.rel_ric[:, 5]*1000.0, label="dC (km/s)")
    plt.xlabel("Time (min)")
    ...
    fig7 = plt.figure()
    r_target = np.linalg.norm(log.target_x_eci[:, :3], axis=1)
    r_chaser = np.linalg.norm(log.chaser_x_eci[:, :3], axis=1)
    alt_target = r_target - re_km
    alt_chaser = r_chaser - re_km
    plt.plot(log.t_s / 60.0, alt_target, label="Target altitude")
    plt.plot(log.t_s / 60.0, alt_chaser, label="Chaser altitude")
    plt.xlabel("Time (min)")
    plt.ylabel("Altitude (km)")
    plt.title("Satellite Altitudes vs Time")
    plt.grid(True)
    plt.legend()
